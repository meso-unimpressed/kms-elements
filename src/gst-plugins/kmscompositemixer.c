...
kms_composite_mixer_recalculate_sizes (gpointer data)
{
  ...
  width = self->priv->output_width / 3;
  height = self->priv->output_height / 3;
  ...
  for (l = values; l != NULL; l = l->next) {
    ...
    top = ((counter / 3) * height);
    left = ((counter % 3) * width);
...
kms_composite_mixer_init (KmsCompositeMixer * self)
{
  ...
  self->priv->output_height = 1080;
  self->priv->output_width = 1080;
  self->priv->n_elems = 0;
